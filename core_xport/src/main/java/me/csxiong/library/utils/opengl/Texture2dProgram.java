...
public class Texture2dProgram {
    ...
    public void draw(float[] mvpMatrix, FloatBuffer vertexBuffer, int firstVertex, int vertexCount, int coordsPerVertex,
        ...
        GLES20.glVertexAttribPointer(maPositionLoc, coordsPerVertex, GLES20.GL_FLOAT, false, vertexStride,
            vertexBuffer);
    ...
    public void draw(float[] glFloat, float[] texFloat, float[] texMatrix, int textureId) {
        // 顶点数组。
        FloatBuffer glPosition = ByteBuffer.allocateDirect(glFloat.length * VertexHelper.BYTES_PER_FLOAT)
            .order(ByteOrder.nativeOrder())
            .asFloatBuffer()
            .put(glFloat);
        glPosition.position(0);
        // 纹理顶点。
        FloatBuffer texturePosition = ByteBuffer.allocateDirect(texFloat.length * VertexHelper.BYTES_PER_FLOAT)
            .order(ByteOrder.nativeOrder())
            .asFloatBuffer()
            .put(texFloat);
        texturePosition.position(0);

        draw(GlUtil.IDENTITY_MATRIX, glPosition, 0, 4, 2, 2 * 4, texMatrix, texturePosition, textureId, 2 * 4);
    }
}
